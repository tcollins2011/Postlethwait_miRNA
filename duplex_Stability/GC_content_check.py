...
def get_args():
    parser = argparse.ArgumentParser(description="returns new csv file with added columns pertaining to GC content")
    parser.add_argument(
        "-f",
        "--file",
        help="csv that was outputed from Process_cofold_results.py",
        required=True,
    ),
    parser.add_argument("-o", "--output", help="desired name of csv file with added GC content columns", required=True)
...
with open(args.file) as fh:
    for line in fh:
        ...
        Sequences = line_list[2].upper()
        Sec_structure = line_list[5]
